...
const checkScreenSize = () => {
  if (
    ...
    window.innerHeight >= 1079 &&
    window.innerHeight <= 1080
  ) {
    document.querySelector(".screenSizeInfo").style.display = "none";
    document.querySelector(".bg").style.display = "block";
  } else {
    document.querySelector(".bg").style.display = "none";
    document.querySelector(".screenSizeInfo").style.display = "block";
  }
};
...
if (!isScreenSizeOK) {
  ...
  const toggleTitle = (zone) => {
    ...
    title.innerHTML = `<p style="margin:6px"> ${getZone(zone)}</p>`;
  };
  ...
  const moveCamera = (value) => {
    ...
    switch (value) {
      ...
      case "btn-b":
        if (!isFunFactON && !isPokedexON && !isStatsON)
          changeZone(previousZone);
